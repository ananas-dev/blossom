...
ifeq ($(UNAME_S), Linux) #LINUX
	ECHO_MESSAGE = "Linux"
	LIBS += $(LINUX_GL_LIBS) -ldl `sdl2-config --libs` `pkg-config --libs freetype2`
	LIBS += -L. -l:libimgui.so

	CXXFLAGS += `sdl2-config --cflags` `pkg-config --cflags freetype2` -Wl,-rpath,.
	CFLAGS = $(CXXFLAGS)
endif
...
$(EXE): $(OBJS) main.cpp libimgui.so plug.h
	$(CXX) -o $@ $^ $(CXXFLAGS) $(LIBS)

clean:
	rm -f $(EXE) $(OBJS) libplug.so
